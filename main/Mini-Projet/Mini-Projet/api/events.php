...
function handleGetParticipants($db, $input) {
    ...
    if (isAdmin()) {
        ...
        if (!$organizer->getProfile()) {
            throw new Exception('Organizer profile not found. Please ensure you are registered as an organizer.');
        }

        // Debug: Check if organizer_id is set
        if (!isset($organizer->organizer_id) || empty($organizer->organizer_id)) {
            throw new Exception('Organizer ID not found. Please contact administrator.');
        }
        ...
        if($participants !== false) {
            ...
            throw new Exception('Failed to load participants or you are not authorized to view this event');
        }
    }
}
